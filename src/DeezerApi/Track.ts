...
export class Track {
    ...
    constructor(ApiResult: TrackApiResult) {
        ...
        this.Title = ApiResult.SNG_TITLE,
        this.Album = ApiResult.ALB_TITLE,
        this.Artist = ApiResult.ART_NAME
    }
    ...
    GetBlowfishKey() {
        ...
        let Md5MusicId = GetMd5Bf(this.Id.toString());
        let BlowfishKey = '';
        for (let i = 0; i < 16; i++) {
...
export const GetTrackById = async (Id: number, User: User) : Promise<Track> => {
    const res = await Axios.post(unofficialApiUrl,
        {
            sng_id: Id
        },
        {
